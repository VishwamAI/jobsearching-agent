...
def add_candidate(first_name, last_name, email, phone=None, resume=None, session=None):
...
def get_candidate_by_email(email, session=None):
    session = session or Session()
    try:
...
def update_candidate(candidate_id, session=None, **kwargs):
    session = session or Session()
    try:
...
def delete_candidate(candidate_id, session=None):
    session = session or Session()
    try:
        candidate = session.query(Candidate).filter_by(id=candidate_id).first()
        if candidate:
...
def add_to_watchlist(candidate_id, job_id, session=None):
    session = session or Session()
    try:
...
def remove_from_watchlist(candidate_id, job_id, session=None):
    session = session or Session()
    try:
        ...
        return None
def schedule_interview(candidate_id, job_id, interview_date, status, session=None):
    session = session or Session()
    try:
...
def update_interview_status(interview_id, status, session=None):
    session = session or Session()
    try:
